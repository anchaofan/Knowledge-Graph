<template>
    <div class="home-container">
        <p class="logo"><img src="../../assets/kg-icon.png" alt="logo"></p>
...
<!--            </div>-->
            <div class="right">
                <el-autocomplete
                        ...
                        v-model="searchText"
                        placeholder="like: 故宫"
                        @select="handleSelect"><!--                        :fetch-suggestions="getSearchListAsync"-->
                  <template slot-scope="{ item }">
                    <div class="name">节点ID： {{ item.id }}</div>
                    <div class="addr">节点名称： {{ item.name }}</div>
                  </template>
                </el-autocomplete>
                <i class="el-icon-search inner-right" @click="getSearchListAsync();"></i>
            </div>
        ...
        <div class="banner-cnt" id="banner-cnt">
            <div class="swipe-cnt">
                <div class="item" @click="getSearchListAsync(item.id)" v-for="item in recommendList" :key="item.id">
    ...
    export default {
        ...
        methods: {
          getSearchListAsync() {
            ...
            this.$axios.get('http://localhost:9090/travel-name?name='  + queryString ).then( (res) => {
              // cb(res.data);

              let data = res.data;
              console.log(data);
              console.log(data[0].name);
              // this.$router.push('/paintingDetail/'+data.name);
              this.$router.push({
                //path:'/paintingDetail',
                name:'paintingDetail',
                //name: "routes[0]",
